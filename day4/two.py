...
def run_game():
    ...
    losingBoards = list(range(0, len(boards)))
    print(losingBoards)
    boardIdx = 0
    ...
    for num in drawnNumbers:
        boardIdx = 0
        while boardIdx < len(boards):
            boards[boardIdx].markNumber(num)
            if boards[boardIdx].checkWin():
                if len(losingBoards) > 1 and boardIdx in losingBoards:
                        losingBoards.remove(boardIdx)
                elif len(losingBoards) == 1 and boards[losingBoards[0]].checkWin():
                    return boards[losingBoards[0]].sum() * num
